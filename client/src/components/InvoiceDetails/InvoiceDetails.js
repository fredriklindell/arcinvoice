...
const InvoiceDetails = () => {
  ...
  const previewPdfHtml = () => {
    ...
    axios
      .post(`${process.env.REACT_APP_API}/invoice-html`, {
        name: invoice.client.name,
        address: invoice.client.address,
        phone: invoice.client.phone,
        email: invoice.client.email,
  ...
  return (
    <div className={cssStyles.PageLayout}>
      {invoice?.creator?.includes(user?._id || user?.googleId) && (
        <div className={cssStyles.buttons}>
          ...
          <button className={cssStyles.btn} onClick={() => previewPdfHtml()}>
            <BorderColorIcon style={iconSize} />
            Preview HTML
          </button>
        </div>
      ...
      {invoiceHtml.length > 0 ? (
        <div
          className={cssStyles.invoiceLayout}
      ...
      <div className={cssStyles.invoiceLayout}>
        ...
        <Container>
          <Grid container justifyContent="space-between" sx={{ mt: 5 }}>
            <Grid item>
              {invoice?.creator?.includes(user?._id) && (
        ...
        <form>
          <div>
              ...
              <Table sx={styles.table} aria-label="simple table">
                ...
                <TableBody>
                  {invoiceData?.items?.map((itemField, index) => (
                    <TableRow key={index}>
                      <TableCell scope="row" sx={{ width: '40%' }}>
                        {' '}
                        <InputBase
                          outline="none"
            ...
            <div className={cssStyles.addButton}></div>
          </div>
