...
const Header = () => {
  ...
  if (!user) return (
    <Box className={cssStyles.header2}>
      <img style={{ width: '160px', cursor: 'pointer' }} onClick={() => navigate('/')} src="https://i.postimg.cc/C5fxh51H/Arc-Invoice-Logo2.png" alt="arc-invoice" />
      <button onClick={() => navigate('/login')} className={cssStyles.login}>Get started</button>
    </Box>
  )
  return (
